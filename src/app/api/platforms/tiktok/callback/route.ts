...
import { getTikTokTokensFromCode } from '@/lib/tiktok/client';
      ...
      const html = `
        ...
        <html>
          ...
          <body>
            <script>
              if (window.opener) {
                window.opener.postMessage({ type: 'oauth_success', platform: 'TIKTOK_ADS' }, '*');
                window.close();
              } else {
                window.location.href = '${returnTo}?tiktok_connected=true';
              }
            </script>
